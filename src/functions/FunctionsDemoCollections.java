	...
	public static void main(String[] args) {
		...
		for (Product product : products) {
			Function<Product, Double> getPriceFunction 
					= product2 -> product2.getPrice(); // product2 is an argument -> returnvalue 
			Function<Double, Double> getDiscountFunction 
					= price -> price * DISCOUNT_RATE;// price is an argument ,-> returnValue
			Function<Product, Double> getPriceAndThenDiscountFunction 
					= getPriceFunction.andThen(getDiscountFunction);// the result of the first funciton is taken as an input argument for  the second function 
			
			Function<Product, Double> getPriceAndThenDiscountFunction2 
					= getDiscountFunction.compose(getPriceFunction); // in case of compose we specify which functipon goes first ; in bith functions the execution oder is the same but the syntaxe is different 
		...
		products.sort(Comparator.comparing(product -> product.getPrice())); // identical lines
